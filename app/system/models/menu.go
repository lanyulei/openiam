...
type Menu struct {
	...
	Auth        json.RawMessage `gorm:"column:auth;type:json;comment:权限标识" json:"auth"`
	Icon        string          `gorm:"column:icon;type:varchar(200);comment:图标" json:"icon"`
	Parent      int             `gorm:"column:parent;comment:父级" json:"parent"`
	Type        int8            `gorm:"column:type;comment:类型" json:"type"` // 1 目录，2 菜单，3 按钮
	Sort        int             `gorm:"column:sort;comment:顺序" json:"sort"`
	AppId       int             `gorm:"column:app_id;comment:应用ID" json:"app_id" binding:"required"`
	models.BaseModel
}
