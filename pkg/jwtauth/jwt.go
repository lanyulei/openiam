...
func GenToken(userInfo *models.UserRequest) (string, error) {
	...
	c := Claims{
		...
		userInfo.IsAdmin,
		jwt.StandardClaims{
			ExpiresAt: time.Now().Add(time.Minute * time.Duration(viper.GetInt("jwt.http.expires"))).Unix(), // 过期时间
	...
	return token.SignedString([]byte(viper.GetString("jwt.http.secret")))
}
